...
#----To change with appropriate players I want to highlight 
...
efg_q4_plot <- filter(efg_merge, fga_total_q13 >= 600 & fga_total_q4 >= 200) %>% 
  ggplot(aes(x = efg_q13, y = efg_q4, label = namePlayer)) +
  geom_point(size = 2) +
  geom_abline(intercept = 0, slope = 1, linetype = "dotted") +
  theme_minimal() +
  labs(title = "Regular season EFG% in the first 3 quarters vs the 4th quarter",
       subtitle = "2016-2020",
       x = "EFG% in quarters 1-3",
       y = "EFG% in 4th quarter") +
  guides(col = FALSE)

ggsave("figures/q4_rs_efg.png", height = 5, width = 7)
